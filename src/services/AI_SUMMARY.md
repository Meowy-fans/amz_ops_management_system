...
**输出**: attribute resolutions、RequirementTree、ResolutionTree、V2 attributes payload、tree-level coverage decisions、review decisions、submission results。

**不变量**: 未审核的 required LLM 属性不得进入 Amazon submitter。

**关键边界**: 不直接跨模块查询其他业务表，不在 service 启动时执行 DDL。

**最后更新**: 2026-06-26 by Codex
