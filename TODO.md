...
| TASK-099 | 设计并实现新品发品 API 出口 | P0 | ✅ | Codex | SP-API Phase 4 | 2026-05-17 | 2026-05-17 | 新增 `AmazonListingsClient.put_listings_item/validation_preview`、`AmazonProductTypeClient`、`AmazonAttributeMapper`（Excel字段→SP-API JSON路径）、`AmazonListingSubmitter`、`ProductListingService.generate_listings_via_api()`、CLI `generate-listing-api`、`sp_api_common.json` 映射配置；12 个新测试；dry-run 19 SKU payload 生成验证通过 |
